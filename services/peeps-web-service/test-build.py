...
build_status, build_output, build_error = run_command("docker build -t local-test .")
print(build_output)
if build_status != 0:
    print(build_error)
    exit(1)
...
run_status, run_output, run_error = run_command("docker run --name local-test-container -d -p 4000:4000 local-test")
if run_status != 0:
...
log_status, log_output, log_error = run_command("docker logs local-test-container 2>&1")

# Check if the container is still running
print("Checking if the container is still running...")
ps_status, ps_output, ps_error = run_command("docker ps | grep local-test-container")
