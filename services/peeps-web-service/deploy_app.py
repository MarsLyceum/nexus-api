...
def run_command(command, env=None):
    print(f"{bcolors.OKBLUE}Running command: {bcolors.BOLD}{command}{bcolors.ENDC}")
...
def main():
    ...
    print(f"{bcolors.OKCYAN}Configuring Docker to use gcloud as a credential helper...{bcolors.ENDC}")
    run_command("gcloud auth configure-docker --quiet")

    # Authenticate gcloud with a service account
    print(f"{bcolors.OKCYAN}Authenticating gcloud with a service account...{bcolors.ENDC}")
    ...
    run_command("DOCKER_BUILDKIT=1 docker build -t gcr.io/hephaestus-418809/hephaestus-api:latest .")
